...
<body style="background-color: #d9d9d9">
                ...
                <tbody>
                    <?php foreach ($soal as $key => $row): ?>
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-primary btn-sm" style="background-color: #ced5e8; color: black"
                                    data-namasoal="<?= $row['soal_nama'] ?>"
                                    onclick="openmodal('<?= $row['soal_id'] ?>', '<?= $row['kategori_id'] ?>', '<?= $row['soal_jenis'] ?>', '<?= $row['no_urut'] ?>')">Edit</button>
                                <a href="hapusSoal.php?id=<?= $row['soal_id'] ?>&kategori_id=<?= $id ?> "
    ...
    <footer class="text-white py-4 fixed-bottom" style="background-color: #2c4182">
    ...
    <div class="modal fade" id="editSurveyModal" tabindex="-1" aria-labelledby="editSurveyModalLabel"
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content bg-white rounded">
                ...
                <div class="modal-body">
                    <form action="saveSoal.php" method="post">
                        <div class="form-group">
                            <label for="editnourut">No Urut</label>
                            <select class="form-select" id="editnourut" name="no_urut">
                                <option value="1">1</option>
                                <option value="2">2</option>
                                <option value="3">3</option>
                                <option value="4">4</option>
                            ...
                            <label for="editsoaljenis">Jenis Soal</label>
                            <select class="form-select" id="editsoaljenis" name="soal_jenis">
                                <option value="Pilihan">Pilihan Ganda</option>
                                <option value="Text">Isian</option>
                            </select>
                            <label for="editkategorisoal">Kategori</label>
                            <select class="form-select" id="editkategorisoal" name="updatekategori_id">
                                <?php foreach ($kategori as $row): ?>
                                    <option value="<?= $row['kategori_id'] ?>"><?= $row['kategori_nama'] ?></option>
                                <?php endforeach; ?>
                            </select>
                            <label for="soalTextarea">Pertanyaan</label>
                            <textarea id="editSurveyTextarea" class="form-control" id="soalTextarea" rows="3"
                                name="soal_nama"></textarea>
                        </div>
                        <input type="hidden" name="soal_id" id="editSurveyId" />
    ...
    <script>
        var inputnama = $('#editSurveyTextarea');
        var inputnourut = document.getElementById('editnourut');
        var inputjenis = document.getElementById('editsoaljenis');
        var inputkategori = document.getElementById('editkategorisoal');
        var inputid = document.getElementById('editSurveyId');

        function openmodal(id, kategori, jenis, nourut) {
            var nama = event.target.getAttribute('data-namasoal');
            inputnama.val(nama);
            inputnourut.value = nourut;
