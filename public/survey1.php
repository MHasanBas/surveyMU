...
if ($getJawaban->rowCount() > 0) {
    header('Location: riwayatsurvey2.php');
}
...
<body style="background-color: #d9d9d9">
    ...
    <div class="container text-center p-3">
        ...
        <p class="fs-3 fw-medium">K.1.2 - KUALITAS FASILITAS POLITEKNIK NEGERI MALANG</p>
        <div class="container w-75 mx-auto bg-white rounded-3 text-left"
            ...
            <p>Halaman berikut berisi pernyataan-pernyataan mengenai yang kamu rasakan dan pikirkan sebagai mahasiswa
                aktif Politeknik Negeri Malang mengenai FASILITAS yang disediakan oleh Politeknik Negeri Malang.</p>
        ...
        <div class="container w-75 mx-auto bg-white rounded-4 text-left p-0 pb-3">
            <div class="m-0" style="background-color: #ced5e8">K.1.2 - KUALITAS FASILITAS POLITEKNIK NEGERI MALANG
            </div>
            <div>
                <form id="submitSurvey1" action="submitsurvey1.php" method="post">
                    <?php if ($length_soal_pilihan > 0): ?>
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="q' . $key . '" id="q' . $key . '_1"
                                value="Sangat Tidak Setuju">
                            <label class="form-check-label" for="q' . $key . '_1">Sangat Tidak Setuju</label>
                        </div>
                    </td>';
                                        echo '<td>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="q' . $key . '" id="q' . $key . '_2"
                                value="Tidak Setuju">
                            <label class="form-check-label" for="q' . $key . '_2">Tidak Setuju</label>
                        </div>
                    </td>';
                                        echo '<td>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="q' . $key . '" id="q' . $key . '_3" value="Setuju">
                            <label class="form-check-label" for="q' . $key . '_3">Setuju</label>
                        </div>
                    </td>';
                                        echo '<td>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="q' . $key . '" id="q' . $key . '_4"
                                value="Sangat Setuju">
                            <label class="form-check-label" for="q' . $key . '_4">Sangat Setuju</label>
                        </div>
                    </td>';
                                        echo '</tr>';
                                        echo '<input type="hidden" name="qid' . $key . '" value="' . $value['soal_id'] . '">';
                                    }
                                }
                                ?>
                            </tbody>
                            ...
                            <tbody>
                                ...
                                foreach ($soal as $key => $value) {
                                    if ($value['soal_jenis'] === "Text") {
                                        ...
                                        echo '<td>' . $value['soal_nama'] . '</td>';
                                        echo '<td>
                    <input name="isian' . $key . '" type="text" class="form-control" style="height: 4rem" />
                    <input type="hidden" name="isianid' . $key . '" value="' . $value['soal_id'] . '">
                    </td>';
                                        echo '</tr>';
                                    }
                                }
                    ...
                    <?php if ($length_soal_pilihan === 0 && $length_soal_text === 0): ?>
                        <p class="fs-5">Tidak ada soal yang tersedia</p>
                    <?php endif; ?>
                    <input type="hidden" name="length_soal_pilihan" value="<?php echo $length_soal_pilihan; ?>">
                    <input type="hidden" name="length_soal_text" value="<?php echo $length_soal_text; ?>">
                </form>
            </div>
        </div>
        <div class="container text-center mt-3">
            <a href="isisurvey.php" class="btn btn-primary btn-lg rounded-pill me-3"
                style="background-color: #5b5b5b; color: white">Kembali</a>
            <?php if ($length_soal_pilihan > 0 || $length_soal_text > 0): ?>
                <button onclick="submitSurvey1()" class="btn btn-primary btn-lg rounded-pill"
                    style="background-color: #4e73df; color: white">Kirim</button>
            <?php endif; ?>
        </div>
    </div>
    ...
    <script>
        var form = $('#submitSurvey1');
        var length_soal_pilihan = <?php echo $length_soal_pilihan; ?>;
        var length_soal_text = <?php echo $length_soal_text; ?>;
        function submitSurvey1() {
            var checked = $('input[type="radio"]:checked').length;
            var text = $('input[type="text"]').val();

            if (checked < length_soal_pilihan) {
                Swal.fire({
                    icon: "error",
                    title: "Oops...",
                    text: "Please fill all the choices!",

                });
            } else if (length_soal_text > 0 && text === '') {
                Swal.fire({
                    icon: "error",
                    title: "Oops...",
                    text: "Please fill all the choices!",
